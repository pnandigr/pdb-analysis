...
def main():
   ...
   #         fs.write('{0} \t {1} \n'.format(index,seq))
    ...
    #-----------------earlier attempt: not quite working

    #resc = np.zeros(int(len(seq_from_pdb)))
    #rescaled = list(resc)
    
    #mod = np.zeros(int(len(seq_from_msa)))
    #mod_indx = list(mod)
    
    #for indx_s,res_s in enumerate(msa_seq):
    #    for indx_p,res_p in enumerate(pdb_seq):
    
    #for indx_s,res_s in enumerate(seq_from_msa):
    #    for indx_p,res_p in enumerate(seq_from_pdb_c):       
    #        if res_p == res_s:
    #            #print(res_p , "\t", res_s, "\t", indx_p, "\t", indx_s)
    #            mod_indx[indx_s] = indx_p
    #            break
    #        else:
    #            next
