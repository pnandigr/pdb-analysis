...
def main():
    ...
    seq_from_pdb = get_coordinates_PDB(PDB_File_In)
    seq_from_msa = msa_seq_lst_c[0]
    #seq_from_pdb_c = "".join([three_to_one(aa3) for aa3 in ["".join(g) for g in zip(*(iter(seq_from_pdb),) * 3)]])
    
    print(seq_from_pdb[0])
    print(seq_from_msa)

mod_indx = []

for indx_p,res_p in enumerate(seq_from_pdb):
